...
void solve() {
  ll n; cin >> n;
  VI c(n+1); rep(i,1,n+1) cin >> c[i];
  VI odd, even;
  odd.push_back(c[1]);
  even.push_back(c[2]);
  ll k = odd[0]*n + even[0]*n;
  ll minodd = odd.back();
  ll mineve = even.back();
  ll sumodd = minodd;
  ll sumeve = mineve;
  ...
  rep(i,3,n+1) {
    ...
    minodd = min(minodd, odd.back());
    mineve = min(mineve, even.back());
    k = min(k, sumodd-minodd + minodd*(n-(lenodd-1)) + sumeve-mineve + mineve*(n-(leneve-1)));
  }
